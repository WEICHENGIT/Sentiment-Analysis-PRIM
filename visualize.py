...
def generate_text(output, states, embed, rnn, neuron_idx, last_layer=-1,
	...
	for t in range(gen_length):
		...
		states, output, c, feat = run_step(x, states, embed, rnn, neuron_idx, 0, overwrite)
		text_out.append(c)
		values.append(feat)
...
def visualize(text, load_model, model_dir, seq_length=256, temperature=0, overwrite=0, neuron=2388,
				logger=None, layer=-1, generate=False, cuda=False, negate = False):
    
	saved_path = ''
	if load_model != '':
		saved_path = os.path.join(model_dir, load_model)
        
	model, recurrent_module, embedder_module, chkpt = make_model(cell_type='mlstm', num_layers=1, embed_size=64, hidden_size=4096, data_size=256, dropout=0, fused=False, weight_norm=False, lstm_only=False, saved_path=saved_path)

	embed = model.module.embedder
	...
	if generate:
		...
		out_text += t_g
		out_values += v_g
		print(''.join(out_text).encode('utf-8').decode())

	save_str = ''
	if logger is not None:
		save_str = logger.get_log_dir('heatmaps')
	save_str = os.path.join(save_str, ''.join(out_text[:10])+'.png')
